...
if (!isset($_SESSION['user_id'])) {
    header("Location: ../public/html/user_login_page.php"); 
    exit;
}
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $cover_dir=__DIR__ . '/../public/uploads/cover_games/';
    $game_dir=__DIR__ . '/../public/uploads/games/';
    ...
    if(!empty($cover)&&$cover['error']===UPLOAD_ERR_OK){
        ...
        if(move_uploaded_file($cover['tmp_name'], $cover_path)){
            $cover_db_path = '../public/uploads/cover_games/' . $cover_name;
        }else{
            echo "Error cover";
            exit;
        }
    ...
    if($game_file['error']===UPLOAD_ERR_OK){
        ...
        if (move_uploaded_file($game_file['tmp_name'], $game_path)) {
            $game_db_path = '../public/uploads/games/' . $game_name;   
        }else{
            echo"Error game";
            exit;
        }
